...
void mergeSort(const std::string &filename, int arr_size, int arity) {
    ...
    if (arr_size <= M) {
        ...
        std::ofstream out("final_result.bin", std::ios::binary);
        if (!out.is_open()) {
    ...
    // Si el tamaño del arreglo no es menor o igual al de memoria principal, se divide en arity bloques
        ...
        // vector con los streams
        std::vector<std::ifstream> a_streams(arity);
        ...
        /*
        // Leer un bloque de cada uno de los archivos
        for (int i = 0; i < arity; i++) {
        ...
        */
        ...
        std::ofstream output("final_result.bin",std::ios::binary);
